################################################################################
#
# Part A: Catch game using Deep Q-Network (DQN) approach
...
class Trainer:
    ...
    def train(self):
        ...
        (
            ...
            rewards,
            terminals,
        ) = self.buffer.sample_trajectories()
...
def save_data(path="partA_results", log=None):
    ...
    os.mkdir(path)
    np.save(
        os.path.join(path, "group_06_catch_rewards_" + log),
        np.array(AVG_TESTING_REWARDS),
    )
    ...
    for e in range(episodes):
        ...
        if e % 10 == 0:
            if validation:
                ...
                if (
                    EARLY_STOPPING
                    and np.min(AVG_TESTING_REWARDS[-N_STOP_EPISODES:]) >= 0.9
                ):
        ...
        print(
            ...
            "/",
            episodes,
            ("Validation Reward: " + str(mean_reward) if not validation else ""),
        )
